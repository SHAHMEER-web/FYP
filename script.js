...
popup.style.backgroundColor = "#f8d7da";  // Light red background (error style)
popup.style.color = "#721c24";  // Dark red text color
...
const knowledgeBase = [
    ...
    },
    {
        ...
        answer: "Hamdard University is located in Karachi, Pakistan.",
    },
    {
        question: "What is the location of Hamdard University?",
        answer: "Hamdard University is located in Karachi, Pakistan.",
    },
    {
...
const unnecessaryQuestions = [
    ...
    // Add more irrelevant questions here
];

// Limiting the number of messages for unnecessary questions
let messageCount = 0;
const maxMessages = 2   ; // Set the maximum number of irrelevant questions
...
function getBestMatch(userQuery) {
    ...
    if (unnecessaryQuestions.some(q => userQueryLower.includes(q.toLowerCase()))) {
        if (messageCount >= maxMessages) {
            // Hide input field and show popup
            userInput.style.display = "none";
            sendButton.style.display = "none";
            popup.style.display = "block"; // Show the error popup
            return ""; // Return empty string to prevent further response
        } else {
            messageCount++;
    ...
    const userQueryVector = tfidf(userQuery); // TF vector for the user's query
    let bestMatch = null;
    let highestSimilarity = 0;

    knowledgeBase.forEach((item) => {
        const questionVector = tfidf(item.question); // TF vector for the knowledge base question
    ...
    // Return the best match's answer or a default response if no match is found
    return bestMatch
        ? bestMatch.answer
...
document.getElementById("closePopup").addEventListener("click", () => {
    location.reload(); // Refresh the page to reset the chatbot and message count
});
